...
def test_get_products():
    ...
    try:
        response = requests.get(url, timeout=5)
        logger.info(f"GET products status: {response.status_code}")
        try:
            response_json = response.json()
            logger.info(f"Products: {json.dumps(response_json, indent=2)}")
        except json.JSONDecodeError:
            logger.info(f"Response text: {response.text}")
        return response.status_code == 200
    except Exception as e:
...
if __name__ == "__main__":
    ...
    get_result = test_get_products()
    
    # Summary
    logger.info("\n=== Test Summary ===\n")
    logger.info(f"Add Product Test: {'PASSED' if add_result else 'FAILED'}")
    logger.info(f"Get Products Test: {'PASSED' if get_result else 'FAILED'}")
    
    if not (add_result and get_result):
        sys.exit(1)